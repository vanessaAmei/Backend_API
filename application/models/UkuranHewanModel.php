...
{
    private $table = 'ukuran_hewan'; //Pilih Table yang akan dijadikan Model

    //Atribut untuk variable tampung, nama bisa beda, disamain agar lebih mudah
    public $id;         
    public $nama;
    public $created_at;
    public $updated_at;
    ...
    //Rule ini digunakan untuk pengecekan apakah sudah sesuai seperti inputan yang tidak valid, kosong
    public $rule = [
        [
            'field' => 'nama',          
            'label' => 'nama',
    ...
    public function update($request,$id) { //Fungsi untuk update data
        ...
        if($this->db->where('id',$id)->update($this->table, $updateData)) //Query Update dimana data nya yaitu $updateData
        {
    ...
    public function destroy($id) { //Fungsi untuk Soft Delete
        ...
        if($this->db->where('id',$id)->update($this->table, $updateData)) //Query Update karena akan melakukan Soft Delete sehingga jangan delete melainkan update
        {
            return ['msg'=>'Berhasil Update Ukuran Hewan','error'=>false];
        }
            return ['msg'=>'Gagal Update Ukuran Hewan','error'=>true];
